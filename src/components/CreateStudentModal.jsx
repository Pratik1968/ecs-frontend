...
function CreateStudentModal({ isOpen, onClose, onStudentCreated, classes, preSelectedClass = null }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(handleApiError(err, 'Creating student'));
    } finally {
      setLoading(false);
    }
  };
        ...
        <CardHeader style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <CardTitle style={{ 
              ...
              <User size={24} />
              {preSelectedClass ? `Add Student to ${preSelectedClass}` : 'Create New Student'}
            </CardTitle>
            <Button
              variant="ghost"
              size="sm"
              ...
              <div>
                ...
                <select
                  ...
                  style={{
                    ...
                    fontSize: '14px',
                    outline: 'none',
                    background: preSelectedClass ? '#f9fafb' : 'white',
                    cursor: preSelectedClass ? 'not-allowed' : 'pointer',
                    transition: 'all 0.2s ease'
                  }}
                  onFocus={(e) => {
                    if (!preSelectedClass) {
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <Button
                    ...
                    style={{
                      background: formData.regNo ? 'linear-gradient(135deg, #10b981 0%, #059669 100%)' : '#f3f4f6',
                      color: formData.regNo ? 'white' : '#9ca3af',
                      border: 'none',
                      borderRadius: '12px',
                      padding: '12px 16px',
                      cursor: formData.regNo ? 'pointer' : 'not-allowed'
                    }}
                  >
                    Generate
                  </Button>
                ...
                <p style={{ 
                  ...
                  Enter registration number first to generate a barcode automatically
                </p>
              </div>
